...
class EncodeGRPCStreaming : EncodeHTTPBase {
  ...
  private val grpcPathByStreamId = ConcurrentHashMap<Int, String>()

  private fun resolveGrpcPathClient(http: Http): String {
    val path = http.path
    val streamIdStr = http.getFirstHeader("X-PacketProxy-HTTP2-Stream-Id")
    if (streamIdStr.isNullOrEmpty()) {
      return path
    }
    val streamId =
      try {
    ...
    if (path == "/trailer-header-frame") {
      val removed = grpcPathByStreamId.remove(streamId)
      return removed ?: path
    }
    if (path == "/data-frame") {
      val mapped = grpcPathByStreamId[streamId]
      return mapped ?: path
    }
    grpcPathByStreamId[streamId] = path
    return path
  }

  private fun resolveGrpcPathServer(http: Http): String {
    val path = http.path
    val streamIdStr = http.getFirstHeader("X-PacketProxy-HTTP2-Stream-Id")
    if (streamIdStr.isNullOrEmpty()) {
      return path
  ...
  @Throws(Exception::class) fun decodeGrpcClientPayload(payload: ByteArray): ByteArray = payload

  @Throws(Exception::class) fun encodeGrpcClientPayload(payload: ByteArray): ByteArray = payload

  @Throws(Exception::class) fun decodeGrpcServerPayload(payload: ByteArray): ByteArray = payload

  @Throws(Exception::class) fun encodeGrpcServerPayload(payload: ByteArray): ByteArray = payload
}
