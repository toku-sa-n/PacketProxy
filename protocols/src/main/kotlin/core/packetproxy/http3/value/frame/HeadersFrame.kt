...
class HeadersFrame private constructor(frameData: ByteArray) : Frame {
  ...
  override fun getBytes(): ByteArray {
    val headersFrameStream = ByteArrayOutputStream()
    try {
      headersFrameStream.write(VariableLengthInteger.of(type).bytes)
      headersFrameStream.write(VariableLengthInteger.of(data.size.toLong()).bytes)
      headersFrameStream.write(data)
    } catch (e: Exception) {
      errWithStackTrace(e)
    }
  ...
  override fun toString(): String =
    String.format("HeadersFrame(data=[%s])", Hex.encodeHexString(data))

  companion object {
    @JvmField val TYPE: Long = 0x01
    ...
    @JvmStatic fun of(frameData: ByteArray): HeadersFrame = HeadersFrame(frameData)
  }
}
