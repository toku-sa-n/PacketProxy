/*
 * Copyright 2019 DeNA Co., Ltd.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package packetproxy.http2.frames

import java.nio.ByteBuffer
import org.apache.commons.codec.binary.Hex
import org.eclipse.jetty.http2.hpack.HpackDecoder
import org.eclipse.jetty.http2.hpack.HpackEncoder
import org.junit.jupiter.api.Test
import packetproxy.http.Http
import packetproxy.util.Logging

class HeadersFrameTest {
  private val encoder: HpackEncoder
  private val decoder: HpackDecoder

  init {
    decoder = HpackDecoder(4096, 4096)
    encoder = HpackEncoder(4096, 4096)
  }

  @Test
  @Throws(Exception::class)
  fun test() {
    val frame =
      Hex.decodeHex(
        "00001B010500000001828487418798E79A82AE43D37A8825B650C3ABB6D2E053032A2F2A".toCharArray()
      )
    val fb = HeadersFrame(frame, decoder)
    Logging.log(fb.toString())
  }

  @Test
  @Throws(Exception::class)
  fun toHttp1Test() {
    val frame =
      Hex.decodeHex(
        "00001B010500000001828487418798E79A82AE43D37A8825B650C3ABB6D2E053032A2F2A".toCharArray()
      )
    val fb = HeadersFrame(frame, decoder)
    Logging.log(String(fb.getHttp()))
  }

  @Test
  @Throws(Exception::class)
  fun constructorHttpTest() {
    val frame =
      Hex.decodeHex(
        "00001B010500000001828487418798E79A82AE43D37A8825B650C3ABB6D2E053032A2F2A".toCharArray()
      )
    val fb = HeadersFrame(frame, decoder)
    val b = fb.getHttp()
    val http = Http.create(b)
    val fb2 = HeadersFrame(http)
    Logging.log(fb)
    Logging.log(fb2)
  }

  @Test
  @Throws(Exception::class)
  fun constructorHttpTest2() {
    val frame =
      Hex.decodeHex(
        ""
          .toCharArray()
      )
    val fb = HeadersFrame(frame, decoder)
    val b = fb.getHttp()
    val http = Http.create(b)
    val fb2 = HeadersFrame(http)
    Logging.log(frame.size)
    Logging.log(fb)
    Logging.log(fb2)
    Logging.log(String(Hex.encodeHex(fb2.toByteArray())))
    Logging.log(String(Hex.encodeHex(fb2.toByteArrayWithoutExtra(encoder))))
  }

  @Test
  @Throws(Exception::class)
  fun bigData() {
    val a =
      Hex.decodeHex(
        ""
          .toCharArray()
      )
    val decoder = HpackDecoder(65535, 65535)
    val bb = ByteBuffer.allocate(4096)
    bb.put(a)
    bb.flip()
    val m = decoder.decode(bb)
    Logging.log(m)
  }

  @Test
  @Throws(Exception::class)
  fun extraDataTest() {
    val frameData =
      Hex.decodeHex(
        ""
          .toCharArray()
      )
    val decoder = HpackDecoder(65535, 65535)
    val headerFrame = HeadersFrame(frameData, decoder)
    Logging.log(String(headerFrame.getExtra()))
  }

  @Test
  @Throws(Exception::class)
  fun initHttpTest() {
    val frameData =
      Hex.decodeHex(
        ""
          .toCharArray()
      )
    val decoder = HpackDecoder(65535, 65535)
    val headerFrame = HeadersFrame(frameData, decoder)
    Logging.log(String(headerFrame.getExtra()))
  }
}
