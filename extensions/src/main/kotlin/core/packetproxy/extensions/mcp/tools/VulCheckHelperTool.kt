package packetproxy.extensions.mcp.tools

import com.google.common.collect.ImmutableList
import com.google.gson.JsonArray
import com.google.gson.JsonObject
import java.util.UUID
import java.util.regex.Matcher
import java.util.regex.Pattern
import packetproxy.VulCheckerManager
import packetproxy.common.Range
import packetproxy.controller.ResendController
import packetproxy.model.Configs
import packetproxy.model.OneShotPacket
import packetproxy.model.Packets
import packetproxy.util.log
import packetproxy.vulchecker.VulChecker
import packetproxy.vulchecker.generator.Generator

/** VulCheck脆弱性テストヘルパーツール 指定されたパケットにVulCheckテストケースを適用して連続送信を実行 */
class VulCheckHelperTool(
  private val packets: Packets,
  private val vulCheckerManager: VulCheckerManager,
  private val resendController: ResendController,
  configs: Configs,
) : AuthenticatedMCPTool(configs) {

  override fun getName(): String = "call_vulcheck_helper"

  override fun getDescription(): String =
    "Execute VulCheck vulnerability tests with automatic payload generation and batch sending. Applies VulCheck test cases to specified packet locations and sends modified packets with configurable intervals. IMPORTANT: Currently supports Number and JWT vulnerability types only. For other vulnerability types, use bulk_send or resend_packet tools instead. For precise targeting, use regex patterns with positive lookahead assertions. Examples: To target '17' in 'X-Version: 17.0.4', use pattern: '17(?=\\.0\\.4)'. To target '123' in 'userId=123&other=456', use pattern: '(?<=userId=)123(?=&)'. To target specific values while preserving structure, use patterns like: 'sessionId=\\\\w+' with replacement 'sessionId=\$1'. The pattern field supports full regex syntax including lookahead/lookbehind assertions for precise matching without affecting surrounding text. Use replacement field to control how the matched text is substituted with VulCheck payloads."

  override fun getInputSchema(): JsonObject {
    var schema = JsonObject()

    var packetIdProp = JsonObject()
    packetIdProp.addProperty("type", "integer")
    packetIdProp.addProperty("description", "ID of the packet to use as base for VulCheck testing")
    schema.add("packet_id", packetIdProp)

    var vulCheckTypeProp = JsonObject()
    vulCheckTypeProp.addProperty("type", "string")
    vulCheckTypeProp.addProperty(
      "description",
      "Type of VulCheck to perform (Number, JWT, etc.). Use 'list' to get available types.",
    )
    schema.add("vulcheck_type", vulCheckTypeProp)

    var targetLocationsProp = JsonObject()
    targetLocationsProp.addProperty("type", "array")
    targetLocationsProp.addProperty(
      "description",
      "Array of target locations in the packet where VulCheck payloads should be injected. Can specify either regex patterns or position ranges.",
    )
    var locationItem = JsonObject()
    locationItem.addProperty("type", "object")
    var locationProps = JsonObject()

    // Regex pattern approach (new)
    var patternProp = JsonObject()
    patternProp.addProperty("type", "string")
    patternProp.addProperty(
      "description",
      "Regex pattern to match target locations in the packet data",
    )
    locationProps.add("pattern", patternProp)

    var replacementProp = JsonObject()
    replacementProp.addProperty("type", "string")
    replacementProp.addProperty(
      "description",
      "Optional replacement template for pattern matches. If not specified, the entire match will be replaced.",
    )
    locationProps.add("replacement", replacementProp)

    // Position range approach (existing - for backward compatibility)
    var startProp = JsonObject()
    startProp.addProperty("type", "integer")
    startProp.addProperty(
      "description",
      "Start position of the target location in the packet data (alternative to pattern)",
    )
    locationProps.add("start", startProp)

    var endProp = JsonObject()
    endProp.addProperty("type", "integer")
    endProp.addProperty(
      "description",
      "End position of the target location in the packet data (alternative to pattern)",
    )
    locationProps.add("end", endProp)

    var descriptionProp = JsonObject()
    descriptionProp.addProperty("type", "string")
    descriptionProp.addProperty("description", "Optional description of this target location")
    locationProps.add("description", descriptionProp)

    locationItem.add("properties", locationProps)
    targetLocationsProp.add("items", locationItem)
    schema.add("target_locations", targetLocationsProp)

    var intervalProp = JsonObject()
    intervalProp.addProperty("type", "integer")
    intervalProp.addProperty(
      "description",
      "Interval between packet sends in milliseconds (default: 100)",
    )
    intervalProp.addProperty("default", 100)
    schema.add("interval_ms", intervalProp)

    var modeProp = JsonObject()
    modeProp.addProperty("type", "string")
    var modeEnum = JsonArray()
    modeEnum.add("sequential")
    modeEnum.add("parallel")
    modeProp.add("enum", modeEnum)
    modeProp.addProperty(
      "description",
      "Execution mode: sequential (with intervals) or parallel (all at once)",
    )
    modeProp.addProperty("default", "sequential")
    schema.add("mode", modeProp)

    var maxPayloadsProp = JsonObject()
    maxPayloadsProp.addProperty("type", "integer")
    maxPayloadsProp.addProperty(
      "description",
      "Maximum number of payloads to generate per location (default: 50, max: 1000)",
    )
    maxPayloadsProp.addProperty("default", 50)
    maxPayloadsProp.addProperty("maximum", 1000)
    schema.add("max_payloads", maxPayloadsProp)

    var timeoutProp = JsonObject()
    timeoutProp.addProperty("type", "integer")
    timeoutProp.addProperty(
      "description",
      "Timeout for entire operation in milliseconds (default: 300000 - 5 minutes)",
    )
    timeoutProp.addProperty("default", 300000)
    schema.add("timeout_ms", timeoutProp)

    // access_tokenを追加
    return addAccessTokenToSchema(schema)
  }

  @Throws(Exception::class)
  override fun executeAuthenticated(arguments: JsonObject): JsonObject {
    log("VulCheckHelperTool called with arguments: " + getSafeArgumentsString(arguments))
    log("VulCheckHelperTool: Starting VulCheck operation")

    // パラメータ取得
    if (!arguments.has("packet_id")) {
      throw IllegalArgumentException("packet_id parameter is required")
    }

    if (!arguments.has("vulcheck_type")) {
      throw IllegalArgumentException("vulcheck_type parameter is required")
    }

    var packetId = arguments.get("packet_id").getAsInt()
    var vulCheckType = arguments.get("vulcheck_type").getAsString()

    // 特別なケース: 利用可能なVulCheckタイプを一覧表示
    if (vulCheckType == "list") {
      return getAvailableVulCheckTypes()
    }

    if (!arguments.has("target_locations")) {
      throw IllegalArgumentException("target_locations parameter is required")
    }

    var targetLocationsJson = arguments.getAsJsonArray("target_locations")
    var intervalMs =
      if (arguments.has("interval_ms")) arguments.get("interval_ms").getAsInt() else 100
    var mode = if (arguments.has("mode")) arguments.get("mode").getAsString() else "sequential"
    var maxPayloads =
      if (arguments.has("max_payloads")) arguments.get("max_payloads").getAsInt() else 50
    var timeoutMs =
      if (arguments.has("timeout_ms")) arguments.get("timeout_ms").getAsInt() else 300000

    log(
      "VulCheckHelperTool: packet_id=$packetId, vulcheck_type=$vulCheckType, locations=" +
        targetLocationsJson.size() +
        ", mode=$mode, max_payloads=$maxPayloads"
    )

    // パケットを取得
    var originalPacket = packets.query(packetId)
    if (originalPacket == null) {
      throw IllegalArgumentException("Packet with ID $packetId not found")
    }

    // OneShotPacketを作成
    var originalOneShot =
      if (originalPacket.getModifiedData().isNotEmpty()) {
        originalPacket.getOneShotFromModifiedData()
      } else if (originalPacket.getSentData().isNotEmpty()) {
        originalPacket.getOneShotPacket(originalPacket.getSentData())
      } else {
        originalPacket.getOneShotFromDecodedData()
      }

    if (originalOneShot == null) {
      throw IllegalArgumentException("Cannot create OneShotPacket from packet ID $packetId")
    }

    // VulCheckerを取得
    var vulChecker = vulCheckerManager.createInstance(vulCheckType)
    if (vulChecker == null) {
      throw IllegalArgumentException(
        "VulCheck type '$vulCheckType' not found. Use 'list' to see available types."
      )
    }

    // ジョブIDを生成
    var jobId = UUID.randomUUID().toString()

    // ターゲット位置を解析
    var targetLocations = parseTargetLocations(targetLocationsJson, originalOneShot.getData())

    var startTime = System.currentTimeMillis()

    // VulCheckテストを実行
    var result =
      executeVulCheckTests(
        originalOneShot,
        vulChecker,
        targetLocations,
        intervalMs,
        mode,
        maxPayloads,
        timeoutMs,
        jobId,
      )

    var executionTime = System.currentTimeMillis() - startTime

    // 結果を構築
    var response = JsonObject()
    response.addProperty("success", result.overallSuccess)
    response.addProperty("vulcheck_type", vulCheckType)
    response.addProperty("mode", mode)
    response.addProperty("total_locations", targetLocations.size)
    response.addProperty("total_payloads_generated", result.totalPayloadsGenerated)
    response.addProperty("total_packets_sent", result.totalPacketsSent)
    response.addProperty("total_failed", result.totalFailed)
    response.addProperty("execution_time_ms", executionTime)
    response.addProperty("job_id", jobId)

    // 各ターゲット位置の結果
    var locationResults = JsonArray()
    for (locResult in result.locationResults) {
      var locJson = JsonObject()
      locJson.addProperty("start", locResult.range!!.getPositionStart())
      locJson.addProperty("end", locResult.range!!.getPositionEnd())
      locJson.addProperty("description", locResult.description)
      locJson.addProperty("payloads_generated", locResult.payloadsGenerated)
      locJson.addProperty("packets_sent", locResult.packetsSent)
      locJson.addProperty("packets_failed", locResult.packetsFailed)
      locJson.addProperty("execution_time_ms", locResult.executionTimeMs)

      // 生成されたペイロードの一覧
      var payloadsList = JsonArray()
      for (payload in locResult.generatedPayloads) {
        payloadsList.add(payload)
      }
      locJson.add("generated_payloads", payloadsList)

      locationResults.add(locJson)
    }
    response.add("location_results", locationResults)

    // パフォーマンス統計
    var performance = JsonObject()
    performance.addProperty("average_interval_ms", result.averageIntervalMs)
    performance.addProperty(
      "payloads_per_second",
      result.totalPacketsSent.toDouble() / (executionTime / 1000.0),
    )
    performance.addProperty(
      "success_rate_percent",
      if (result.totalPacketsSent > 0)
        ((result.totalPacketsSent - result.totalFailed).toDouble() / result.totalPacketsSent) *
          100.0
      else 0.0,
    )
    response.add("performance", performance)

    log(
      "VulCheckHelperTool: Completed. Generated " +
        result.totalPayloadsGenerated +
        " payloads, sent " +
        result.totalPacketsSent +
        " packets, " +
        result.totalFailed +
        " failed, time: ${executionTime}ms"
    )

    return response
  }

  /** 利用可能なVulCheckタイプを取得 */
  @Throws(Exception::class)
  private fun getAvailableVulCheckTypes(): JsonObject {
    var manager = vulCheckerManager
    var vulCheckerNames = manager.getVulCheckerNameList()

    var result = JsonObject()
    result.addProperty("available_vulcheck_types", vulCheckerNames.joinToString(", "))

    var typesArray = JsonArray()
    var typeDetails = HashMap<String, JsonObject>()

    for (name in vulCheckerNames) {
      var checker = manager.createInstance(name)
      if (checker != null) {
        var typeInfo = JsonObject()
        typeInfo.addProperty("name", name)
        typeInfo.addProperty("description", "VulCheck tests for $name vulnerabilities")

        // ジェネレータの詳細を追加
        var generators: ImmutableList<Generator> = checker.getGenerators()
        var generatorsList = JsonArray()
        for (gen in generators) {
          var genInfo = JsonObject()
          genInfo.addProperty("name", gen.getName())
          genInfo.addProperty("generate_on_start", gen.generateOnStart())
          generatorsList.add(genInfo)
        }
        typeInfo.add("generators", generatorsList)
        typeInfo.addProperty("generator_count", generators.size)

        typesArray.add(typeInfo)
        typeDetails[name] = typeInfo
      }
    }

    result.add("vulcheck_types", typesArray)
    result.addProperty("total_types", vulCheckerNames.size)

    return result
  }

  /** ターゲット位置を解析してTargetLocationリストに変換 */
  @Throws(Exception::class)
  private fun parseTargetLocations(
    targetLocations: JsonArray,
    packetData: ByteArray,
  ): List<TargetLocation> {
    var locations = ArrayList<TargetLocation>()
    var packetStr = String(packetData)

    for (element in targetLocations) {
      var location = element.getAsJsonObject()

      // Check if regex pattern is specified
      if (location.has("pattern")) {
        // Regex-based approach
        var pattern = location.get("pattern").getAsString()
        var replacement =
          if (location.has("replacement")) location.get("replacement").getAsString() else null
        var description =
          if (location.has("description")) location.get("description").getAsString()
          else ("Pattern: $pattern")

        // Find all matches for the pattern
        var regex = Pattern.compile(pattern)
        var matcher = regex.matcher(packetStr)

        var matchCount = 0
        while (matcher.find()) {
          matchCount++
          var start = matcher.start()
          var end = matcher.end()

          var targetLoc = TargetLocation()
          targetLoc.range = Range.of(start, end)
          targetLoc.description = "$description (match $matchCount)"
          targetLoc.pattern = pattern
          targetLoc.replacement = replacement
          targetLoc.originalMatch = matcher.group()

          locations.add(targetLoc)
        }

        if (matchCount == 0) {
          log("VulCheckHelperTool: No matches found for pattern: $pattern")
        }
      } else if (location.has("start") && location.has("end")) {
        // Position-based approach (backward compatibility)
        var start = location.get("start").getAsInt()
        var end = location.get("end").getAsInt()
        var description =
          if (location.has("description")) location.get("description").getAsString()
          else ("Range $start-$end")

        if (start < 0 || end < start || end > packetData.size) {
          throw IllegalArgumentException(
            "Invalid range: start=$start, end=$end, packet length=${packetData.size}"
          )
        }

        var targetLoc = TargetLocation()
        targetLoc.range = Range.of(start, end)
        targetLoc.description = description
        targetLoc.originalMatch = packetStr.substring(start, end)

        locations.add(targetLoc)
      } else {
        throw IllegalArgumentException(
          "Each target location must have either 'pattern' or both 'start' and 'end' properties"
        )
      }
    }

    if (locations.isEmpty()) {
      throw IllegalArgumentException("No valid target locations found")
    }

    return locations
  }

  /** VulCheckテストを実行 */
  @Throws(Exception::class)
  private fun executeVulCheckTests(
    originalPacket: OneShotPacket,
    vulChecker: VulChecker,
    targetLocations: List<TargetLocation>,
    intervalMs: Int,
    mode: String,
    maxPayloads: Int,
    timeoutMs: Int,
    jobId: String,
  ): VulCheckResult {
    var result = VulCheckResult()
    result.overallSuccess = true

    // 各ターゲット位置に対してテストを実行
    for (locationIndex in targetLocations.indices) {
      var targetLocation = targetLocations[locationIndex]

      log(
        "VulCheckHelperTool: Processing target location " +
          (locationIndex + 1) +
          "/" +
          targetLocations.size +
          " at range " +
          targetLocation.range!!.getPositionStart() +
          "-" +
          targetLocation.range!!.getPositionEnd() +
          " (" +
          targetLocation.description +
          ")"
      )

      var locResult =
        processTargetLocation(
          originalPacket,
          vulChecker,
          targetLocation,
          intervalMs,
          mode,
          maxPayloads,
          jobId,
        )

      result.locationResults.add(locResult)
      result.totalPayloadsGenerated += locResult.payloadsGenerated
      result.totalPacketsSent += locResult.packetsSent
      result.totalFailed += locResult.packetsFailed

      if (locResult.packetsFailed > 0) {
        result.overallSuccess = false
      }
    }

    // 平均間隔を計算
    if (result.totalPacketsSent > 1) {
      result.averageIntervalMs = intervalMs.toDouble() // sequentialモードの場合
    }

    return result
  }

  /** 特定のターゲット位置でVulCheckテストを実行 */
  @Throws(Exception::class)
  private fun processTargetLocation(
    originalPacket: OneShotPacket,
    vulChecker: VulChecker,
    targetLocation: TargetLocation,
    intervalMs: Int,
    mode: String,
    maxPayloads: Int,
    jobId: String,
  ): LocationResult {
    var result = LocationResult()
    result.range = targetLocation.range
    result.description = targetLocation.description

    var startTime = System.currentTimeMillis()

    // 元のパケットデータを取得
    var originalData = originalPacket.getData()
    var originalText = String(originalData)

    // ターゲット範囲のデータを取得
    if (targetLocation.range!!.getPositionEnd() > originalData.size) {
      throw IllegalArgumentException(
        "Target range exceeds packet data length: range end=" +
          targetLocation.range!!.getPositionEnd() +
          ", data length=" +
          originalData.size
      )
    }

    var targetData =
      targetLocation.originalMatch
        ?: originalText.substring(
          targetLocation.range!!.getPositionStart(),
          targetLocation.range!!.getPositionEnd(),
        )

    // VulCheckのジェネレータを取得してペイロードを生成
    var generators: ImmutableList<Generator> = vulChecker.getGenerators()

    var payloadCount = 0
    var sentCount = 0
    var failedCount = 0

    for (generator in generators) {
      if (payloadCount >= maxPayloads) {
        log("VulCheckHelperTool: Reached max payload limit ($maxPayloads)")
        break
      }

      try {
        var payload = generator.generate(targetData)
        if (payload != null && payload != targetData) {
          result.generatedPayloads.add(payload)
          payloadCount++

          // パケットを改変して送信
          var modifiedText: String
          if (targetLocation.pattern != null && targetLocation.replacement != null) {
            // Regex pattern replacement approach
            var replacementTemplate = targetLocation.replacement!!.replace("\$1", payload)
            var pattern = Pattern.compile(targetLocation.pattern)
            var matcher = pattern.matcher(originalText)

            // Find the specific match we're targeting
            var sb = StringBuffer()
            while (matcher.find()) {
              if (matcher.start() == targetLocation.range!!.getPositionStart()) {
                // This is our target match
                matcher.appendReplacement(sb, Matcher.quoteReplacement(replacementTemplate))
                break
              } else {
                // Keep other matches unchanged
                matcher.appendReplacement(sb, Matcher.quoteReplacement(matcher.group()))
              }
            }
            matcher.appendTail(sb)
            modifiedText = sb.toString()
          } else {
            // Position-based replacement approach
            modifiedText =
              originalText.substring(0, targetLocation.range!!.getPositionStart()) +
                payload +
                originalText.substring(targetLocation.range!!.getPositionEnd())
          }

          var modifiedData = modifiedText.toByteArray()

          // ジョブ情報を付与してパケット作成
          var temporaryId = UUID.randomUUID().toString()
          var modifiedPacket =
            OneShotPacket(
              originalPacket.getId(),
              originalPacket.getListenPort(),
              originalPacket.getClient(),
              originalPacket.getServer(),
              originalPacket.getServerName()!!,
              originalPacket.getUseSSL(),
              modifiedData,
              originalPacket.getEncoder()!!,
              originalPacket.getAlpn()!!,
              originalPacket.getDirection()!!,
              originalPacket.getConn(),
              originalPacket.getGroup(),
              jobId,
              temporaryId,
            )

          // 送信モードに応じて処理
          if (mode == "parallel") {
            // 並列送信 - すぐに送信
            try {
              resendController.resend(modifiedPacket)
              sentCount++
            } catch (e: Exception) {
              log("VulCheckHelperTool: Failed to send packet with payload: " + e.message)
              failedCount++
            }
          } else {
            // 順次送信 - 間隔を設けて送信
            try {
              resendController.resend(modifiedPacket)
              sentCount++

              if (intervalMs > 0 && payloadCount < maxPayloads && payloadCount < generators.size) {
                Thread.sleep(intervalMs.toLong())
              }
            } catch (e: Exception) {
              log("VulCheckHelperTool: Failed to send packet with payload: " + e.message)
              failedCount++
            }
          }
        }
      } catch (e: Exception) {
        log(
          "VulCheckHelperTool: Failed to generate payload with generator " +
            generator.getName() +
            ": " +
            e.message
        )
        failedCount++
      }
    }

    result.payloadsGenerated = payloadCount
    result.packetsSent = sentCount
    result.packetsFailed = failedCount
    result.executionTimeMs = System.currentTimeMillis() - startTime

    log(
      "VulCheckHelperTool: Location complete - generated $payloadCount payloads, sent $sentCount packets, $failedCount failed"
    )

    return result
  }

  /** VulCheckテストの全体結果 */
  private class VulCheckResult {
    var overallSuccess = true
    var totalPayloadsGenerated = 0
    var totalPacketsSent = 0
    var totalFailed = 0
    var averageIntervalMs = 0.0
    var locationResults: MutableList<LocationResult> = ArrayList()
  }

  /** ターゲット位置の情報 */
  private class TargetLocation {
    var range: Range? = null
    var description: String? = null
    var pattern: String? = null // regex pattern (if used)
    var replacement: String? = null // replacement template (if used)
    var originalMatch: String? = null // original matched text
  }

  /** 特定位置でのテスト結果 */
  private class LocationResult {
    var range: Range? = null
    var description: String? = null
    var payloadsGenerated = 0
    var packetsSent = 0
    var packetsFailed = 0
    var executionTimeMs: Long = 0
    var generatedPayloads: MutableList<String> = ArrayList()
  }
}
