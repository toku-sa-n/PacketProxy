...
class GUIOptionServerDialog(owner: JFrame) : JDialog(owner) {
  ...
  private val ssl = JCheckBox(I18nString.get("Need a SSL/TLS to connect"))
  private val dns = JCheckBox(I18nString.get("Spoofing A Record"))
  private val dns6 = JCheckBox(I18nString.get("Spoofing AAAA Record"))
  private val upstream = JCheckBox(I18nString.get("Need to be defined as an Upstream Http Proxy"))
  private val encoders = JComboBox<String>()
  private val descriptor = JButton(I18nString.get("Import Proto File"))
  ...
  init {
    title = I18nString.get("Server setting")
    val rect = owner.bounds
    setBounds(rect.x + rect.width / 2 - 350, rect.y + rect.height / 2 - 290, 700, 580)
    EncoderManager.getInstance().getEncoderNameList().forEach { encoders.addItem(it) }
    encoders.maximumRowCount = encoders.itemCount
    encoders.selectedItem = "HTTP"
    val panel = JPanel()
    panel.layout = BoxLayout(panel, BoxLayout.Y_AXIS)
    panel.add(labeled(I18nString.get("Server name:"), ip))
    panel.add(labeled(I18nString.get("Server port:"), port))
    panel.add(labeled(I18nString.get("Use SSL/TLS:"), ssl))
    panel.add(labeled(I18nString.get("Encode module:"), encoders))
    descriptorPanel = JPanel()
    descriptorPanel.layout = BoxLayout(descriptorPanel, BoxLayout.X_AXIS)
    descriptorPanel.add(JLabel(I18nString.get("gRPC descriptor (.desc):")))
    descriptorPanel.add(descriptor)
    descriptorPanel.add(Box.createHorizontalGlue())
    panel.add(descriptorPanel)
    panel.add(
      labeled(
        I18nString.get("DNS Spoofing:"),
    ...
    panel.add(labeled(I18nString.get("Upstream HTTP Proxy:"), upstream))
    panel.add(labeled(I18nString.get("Comments:"), comment))
    val footer = JPanel()
    ...
    descriptor.addActionListener {
      try {
        val outcome =
          GUIOptionGrpcDescriptorDialog(owner, editingServerId, descriptorPath).showManageDialog()
  ...
  private fun save() {
    if (ip.text.any { it.code !in 0x21..0x7e }) {
      JOptionPane.showMessageDialog(
        null,
        I18nString.get("The ServerName contains invalid characters."),
      )
      return
    }
    result =
