...
open class JWTHeaderJKUModifiedByAtmarkGenerator : Generator() {
  override fun getName(): String = "Header: jku: @を利用して制御可能な別のドメインに変更"
  ...
  override fun generate(inputData: String): String {
    ...
    val jwt = JWTAlgRS256(inputData, secretKey)
    val jku = jwt.getHeaderValue("jku")
    jwt.setHeaderValue("alg", "RS256")
    jwt.setHeaderValue(
      "jku",
      String.format("%s%s", jku ?: "https://userinfo", "@certs.funacs.com/"),
    )
