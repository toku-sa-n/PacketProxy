...
object FrameUtils {
  @JvmField var PREFACE: ByteArray = byteArrayOf()
  @JvmField var SETTINGS: ByteArray = byteArrayOf()
  @JvmField var END_SETTINGS: ByteArray = byteArrayOf()
  @JvmField var WINDOW_UPDATE: ByteArray = byteArrayOf()
  ...
  fun parseFrames(framesIn: ByteArray?, hpackDecoder: HpackDecoder?): List<Frame> {
    ...
    while (frames != null && frames.isNotEmpty()) {
      ...
      if (!isPreface(frame)) frameList.add(FrameFactory.create(frame, hpackDecoder))
    }
    return frameList
  }
}
