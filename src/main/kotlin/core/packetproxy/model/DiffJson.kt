...
class DiffJson private constructor() : DiffBase() {
  ...
  companion object {
    private var instance: DiffJson? = null

    @JvmStatic
    @Throws(Exception::class)
    fun getInstance(): DiffJson {
      if (instance == null) {
        instance = DiffJson()
      }
      return instance!!
        ...
        for (delta in deltas) {
          ...
          if (delta.getType() == Delta.TYPE.CHANGE) {
            original_event.foundChgDelta(
              chunkPositionPerCharacter(listOrig, chunkOrig),
              chunkLengthPerCharacter(chunkOrig),
            )
            target_event.foundChgDelta(
              chunkPositionPerCharacter(listTarg, chunkTarg),
    ...
    fun diffPerLine(
      ...
      try {
        ...
        for (delta in deltas) {
          ...
          if (delta.getType() == Delta.TYPE.CHANGE) {
            original_event.foundChgDelta(
              chunkPositionPerLine(listOrig, chunkOrig),
              chunkLengthPerLine(chunkOrig),
            )
            target_event.foundChgDelta(
              chunkPositionPerLine(listTarg, chunkTarg),
